...
use crate::{args::Arguments, qvd_structure::{QvdFieldHeader, QvdTableHeader}};
use std::{
    io::{SeekFrom, self, Read, BufRead, Seek},
    path::Path,
    str,
    collections::HashMap, 
    fs::File,
...
pub fn read_qvd(args: Arguments) {
    ...
    if let Ok(f) = File::open(&args.path) {
        ...
        let data: Vec<Data> = qvd_structure.fields.headers.iter().map(|field| {
            symbol_map.insert(
...
fn create_dataframe(data: Vec<Data>) -> DataFrame {
    let series: Vec<Series> = data
        .iter()
        .map(|col| Series::new(&col.column, &col.values[..]))
        .collect();
    DataFrame::new(series).expect("A dataframe to be created")
}

pub fn read_qvd_metadata(args: Arguments) {
    let xml: String = get_xml_data(&args.path).expect("Error reading file");
    let qvd_structure: QvdTableHeader = from_str(&xml).unwrap();
